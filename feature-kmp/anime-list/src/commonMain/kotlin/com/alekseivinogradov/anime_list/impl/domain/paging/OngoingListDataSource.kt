...
import com.alekseivinogradov.anime_base.api.domain.DEFAULT_PAGING_PAGE
...
class OngoingListDataSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ListItemDomain> {
        val page = params.key ?: DEFAULT_PAGING_PAGE
        ...
        return when (usecaseResult) {
            is CallResult.Success -> LoadResult.Page(
                ...
                prevKey = if (page <= DEFAULT_PAGING_PAGE) null else (page - 1),
                nextKey = page + 1
            )
