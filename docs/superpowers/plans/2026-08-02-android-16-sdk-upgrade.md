...
| JobScheduler runtime quota based on standby bucket (targetSdk 36) | Applies — `WorkManager` runs on `JobScheduler`, so this is not exempted by "uses WorkManager". `feature-platform/anime-background-update/.../AnimePeriodicBackgroundUpdateModule.kt` schedules an hourly unconstrained `PeriodicWorkRequest` (no `NetworkType.CONNECTED` constraint) doing sequential paged network calls | **Accepted — low impact** | An over-quota execution is cut short and retried via WorkManager's backoff policy, not a crash or data loss. Worth adding a network constraint to this worker as a follow-up, out of scope for this branch. |
